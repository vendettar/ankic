"""Basic parser tests for saved Vocabulary.com HTML fixtures.

Note: The main application never parses local HTML at runtime; this test loads
fixtures to validate the parser against real pages.
"""
...
def load_fixture(name: str) -> BeautifulSoup:
    # Look for test fixtures in various locations
    here = Path(__file__).resolve().parent
    candidates = [
        here
        / "source"
        / f"vocab_word_{name.lower()}.html",  # tests/source/vocab_word_<name>.html
        here
        / "source"
        / f"vocab_word_{name.lower()}_ajax_result.html",  # tests/source/vocab_word_<name>_ajax_result.html
        here / "html" / f"{name}.html",  # tests/html/<Name>.html (legacy)
        here / f"{name}.html",  # tests/<Name>.html (legacy)
        here.parent / f"{name}.html",  # project root <Name>.html (legacy)
    ]
    for p in candidates:
        if p.exists():
...
def test_nebulous_fixture_parses():
    soup = load_fixture("Nebulous")
    f = VocabularyFetcher()
    data = f._parse_vocab_soup(soup)
    assert_common_structure(data)
    # Should include at least one definition
    assert data["parts"], "No definitions parsed from Nebulous.html"


def test_up_fixture_parses():
    soup = load_fixture("Up")
    f = VocabularyFetcher()
    data = f._parse_vocab_soup(soup)
    assert_common_structure(data)
    # Up has many senses; ensure multiple parts
    assert len(data["parts"]) >= 2


def test_tumult_fixture_parses():
    soup = load_fixture("Tumult")
    f = VocabularyFetcher()
    data = f._parse_vocab_soup(soup)
    assert_common_structure(data)
    # Ensure we didn't incorrectly mix 'types' / 'type of' into synonyms
    for part in data["parts"]:
        syns = part.get("synonyms") or []
        assert all(s.lower() not in ("types", "type of") for s in syns)
...
def test_project_ajax_fixture_parses():
    ...
    assert len(data["parts"]) >= 5, f"Project should have at least 5 different definitions, got {len(data['parts'])}"
    ...
    assert noun_count >= 3, f"Project should have at least 3 noun definitions, got {noun_count}"
    assert verb_count >= 5, f"Project should have at least 5 verb definitions, got {verb_count}"

    # Check that we have phonetics information
    phonetics = data.get("phonetics", [])
    print(f"Found phonetics: {phonetics}")
    assert len(phonetics) >= 2, f"Project should have at least 2 pronunciations, got {len(phonetics)}"
