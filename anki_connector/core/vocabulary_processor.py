...
class VocabularyProcessor:
    ...
    def convert_to_card_data(self, word_info: WordInfo) -> dict[str, str]:
        ...
        card_data = {
            "Word": word_info.word,
            "US_Phonetic": word_info.phonetics.us or "",
            "UK_Phonetic": word_info.phonetics.uk or "",
            "US_Audio": "",
            "UK_Audio": "",
            "vocab_WordForms": ", ".join(word_info.word_forms.forms),
            "vocab_ShortExplanation": word_info.short_explanation or "",
            "vocab_LongExplanation": word_info.long_explanation or "",
            "Etymology": "",
            "Tags": "vocabulary",
        }

        # Add definition fields (up to 25)
        for i in range(1, 26):
            card_data[f"vocab_Part{i}"] = ""
            card_data[f"vocab_Definition{i}"] = ""

        # Fill in definitions
        for i, definition in enumerate(word_info.definitions[:25], 1):
            card_data[f"vocab_Part{i}"] = self.text_processor.abbreviate_part_of_speech(
            ...
            card_data[f"vocab_Definition{i}"] = definition_text
        ...
        try:
            ...
            if include_audio and settings.audio.enable_audio:
                ...
                if audio_files:
                    ...
                    card_data["US_Audio"] = anki_audio.us_audio or ""
                    card_data["UK_Audio"] = anki_audio.uk_audio or ""
