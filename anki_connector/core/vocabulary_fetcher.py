...
class VocabularyFetcher(VocabularyFetcherInterface):
    ...
    def __init__(
        ...
        self.base_url = base_url or VocabularyConstants.VOCABULARY_BASE_URL
        self.timeout = int(
        ...
        for section in sections:
            for block in section.select(".ipa-with-audio"):
                ...
                if label:
                    ...
                    results.append(ipa)
        # Deduplicate by actual phonetic content, not by label
        seen_phonetics = set()
        out: list[str] = []
        for r in results:
            # Extract the actual phonetic part (everything after the last colon, or the whole string)
            if ":" in r:
                phonetic_part = r.split(":", 1)[1].strip()
            else:
                phonetic_part = r.strip()

            if phonetic_part not in seen_phonetics:
                out.append(r)
                seen_phonetics.add(phonetic_part)
        return out
        ...
        for wrap in soup.select(".word-definitions"):
            for li in wrap.select("ol > li"):
                ...
                for child in def_el.children:
                    ...
                    parts.append(get_text(strip=True) if callable(get_text) else str(child))
    ...
    def _extract_word_forms(self, soup: BeautifulSoup) -> list[str]:
        ...
        for pf in soup.select("p.word-forms"):
            vals = [b.get_text(strip=True) for b in pf.select("b")]
            if not vals:
    ...
    def _is_no_result_page(self, soup: BeautifulSoup) -> bool:
        """Detect vocabulary.com suggestion/no-result pages to avoid caching noise."""
        text = soup.get_text(" ")
        if not text:
            return False
        return any(
            signal.lower() in text.lower()
            for signal in VocabularyConstants.NO_RESULT_SIGNALS
        )
    ...
    # Removed heuristic form derivation to avoid incorrect forms; rely on site data only.
...
# (Removed deprecated convenience function)
